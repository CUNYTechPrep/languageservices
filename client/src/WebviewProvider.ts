...
export class DiffWebviewProvider {
	...
	public static createOrShow(extensionUri: vscode.Uri, diffData: DiffData) {
		...
		DiffWebviewProvider.currentPanel = new DiffWebviewProvider(panel, extensionUri);
		DiffWebviewProvider.currentPanel._updateContent(diffData);
	}
	...
	private async _acceptChanges(data: { modifiedContent?: string }) {
		try {
			...
			const document = await vscode.workspace.openTextDocument(this._diffData.targetFile);
	...
	private _getHtmlForWebview(): string {
		...
		const styleUri = this._panel.webview.asWebviewUri(
			vscode.Uri.joinPath(this._extensionUri, 'client', 'media', 'diff-viewer.css')
		);
		const scriptUri = this._panel.webview.asWebviewUri(
			vscode.Uri.joinPath(this._extensionUri, 'client', 'media', 'diff-viewer.js')
		);
