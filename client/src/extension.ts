...
export function activate(context: ExtensionContext) {
				...
				const placeholderSchema = {
					properties: {
						...
						correct: {
							type: 'boolean',
							description:
								'When set to true, LLM will correct given data and directly replace original data',
						},
					},
				};
	...
	const executeYamlActionsCommand = vscode.commands.registerCommand(
		'extension.executeYamlActions',
		async () => {
			const editor = vscode.window.activeTextEditor;
			if (!editor) {
			...
			if (!yamlText) {
				vscode.window.showErrorMessage(
					'Couldnt get YAML content. Ensure this is a .yaml file perhaps.'
				);
				return;
			}
				...
				async (progress, token) => {
					try {
						...
						if (response.success) {
							...
							const correctionResult = response.results?.find(
								r => r.key === 'correct' && r.result.corrected
							);
							...
							if (correctionResult) {
								...
								try {
									...
									parsedYaml.data = correctionResult.result.corrected.replace(
										/^`|`$/g,
										''
								...
								vscode.window.showInformationMessage(
									'YAML actions executed successfully'
								);
							}
							...
							vscode.window.showErrorMessage(
								'Failed to execute YAML actions: ' +
									(response.error || 'Unknown error')
							);
						}
					} catch (error) {
						vscode.window.showErrorMessage(
							`Error executing YAML actions: ${error.message}`
						);
					}
				}
			);
